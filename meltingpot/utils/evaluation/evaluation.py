...
class ReturnSubject(subject.Subject):
  ...
  def on_next(self, timestep: dm_env.TimeStep):
    ...
    if not timestep.reward == ():
      self.reward_list.append(np.stack(timestep.reward))
    if timestep.step_type.last():
      super().on_next(self._return)
      super().on_next(self.reward_list)
      self._return = None
      self.reward_list = []
...
def run_and_observe_episodes(
  ...
  with contextlib.ExitStack() as stack:
    ...
    for n in range(num_episodes):
      action_list = run_episode(population, substrate)
      logging.info('%4d / %4d episodes completed...', n + 1, num_episodes)
  data_reorganized = data.copy()
  data_reorganized['focal_player_returns'] = data['focal_player_returns'][0::2]
  # reward = np.stack(data['focal_player_returns'][1::2]).squeeze()
  data_reorganized['reward'] = data['focal_player_returns'][1::2]
